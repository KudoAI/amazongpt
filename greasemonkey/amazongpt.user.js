...
(async () => {
    ...
    const modals = {
        ...
        about: {
            show() {
                ...
                aboutHeaderLogo.style.cssText = `max-width: 98% ; margin: -5px ${ isMobile ? 'auto' : '15.5%' } 17px`
    ...
    const update = {
        ...
        appStyle() {
            appStyle.innerText = (
              ...
              + '#app-chatbar {'
                  ...
                  + 'font-size: 0.92rem ; height: 46px ; width: 100% ; max-height: 200px ; resize: none ; '
        ...
        scheme(newScheme) {
            scheme = newScheme ; logos.amzgpt.update() ; update.appStyle() ; update.stars() ; toggle.btnGlow() ;
        ...
        tooltip(buttonType) { // text & position
            const cornerBtnTypes = ['chevron', 'about', 'settings', 'speak', 'font-size', 'arrows']
        ...
        reply(answer) {
            ...
            if (!appDiv.querySelector('pre')) {
                ...
                if (!isMobile) // add hover listeners for tooltips
                    [aboutSpan, settingsSpan, chevronSpan, speakerSpan, fontSizeSpan, arrowsSpan].forEach(span => {
            ...
            if (config.autoScroll && !isMobile && config.proxyAPIenabled && !config.streamingDisabled)
                window.scrollBy({ top: appDiv.querySelector('#app-chatbar').getBoundingClientRect().bottom - window.innerHeight +12 })
    ...
    const appDiv = document.createElement('div') ; appDiv.id = 'amzgpt' ; appDiv.classList.add('fade-in')
    ...
    ['wsbg', 'bsbg'].forEach(cssType => // code highlighting, white stars, black stars
